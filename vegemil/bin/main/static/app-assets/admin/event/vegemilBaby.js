...
$(function () {
  ...
  $('#btnDel').click(function(e){
    ...
    if(confirm('삭제하시겠습니까?')){
		$.ajax({
			url : '/admin/manage/event/deleteVegemilBabyEvent',
			type : "post",
			data : $(form).serialize(),
			dataType : "json",
...
//	$('#sEnddate').attr("value",enddate);
    var dt_basic = dt_basic_table.DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              '<button type="button" class="btn btn-primary btn-sm btn-sm waves-effect waves-float waves-light" onclick="location.href = \'eventUpdateVegemilBaby?eIdx='+full['eIdx']+ '\'">수정</button>'
            );
          }
    ...
    $('div.head-label').html('<h4 class="card-title">이벤트 목록 <button type="button" id="btnDel" class="btn btn-outline-danger btn-sm me-1">선택삭제</button>'+
                             '<button type="button" onclick="location.href=\'/admin/manage/event/eventAddVegemilBaby\'" class="btn btn-outline-info btn-sm me-1">새글등록</button></h4>');							 
...
function btnDisplay(idx, idx1, idx2) {
	...
	if(confirm('진열을 수정하시겠습니까?')){		
		$.ajax({
			url : '/admin/manage/event/displayEventInfoVB?eIdx='+eIdx+'&eActive='+eActive+'&eBvactive='+eBvactive,			
			type : "get",
			dataType : "json",
			success : function(data) {
				console.log('data============',data);
