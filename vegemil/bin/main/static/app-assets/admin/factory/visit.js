...
$(function () {
    ...
    $('#btnDel').click(function(e){
	    ...
	    if(confirm('삭제하시겠습니까?')){
			$.ajax({
				url : '/admin/manage/factory/deleteVisit',
				type : "post",
				data : $(form).serialize(),
				dataType : "json",
...
var createTable = function() {
  ...
  if (dt_basic_table.length) {
    ...
    var dt_basic = dt_basic_table.DataTable({
	  ...
	  processing: true,
      ajax: {
        url : '/admin/manage/factory/visitList',
        dataType : 'json',
...
function btnSave(idx) {
    ...
    $.ajax({
       url: '/admin/manage/factory/saveVisit',
...
function btnDisplay(idx) {
	...
	$.ajax({
		url : '/admin/manage/factory/displayVisit?vIdx='+idx+'&vDisplay='+vDisplay,
		type : "get",
		dataType : "json",
		success : function(data) {
...
function btnSetupSave() {
    ...
    $.ajax({
       url: '/admin/manage/factory/saveVisitSetup',
	   processData: false,  // 데이터 객체를 문자열로 바꿀지에 대한 값이다. true면 일반문자...
	   contentType: false,  // 해당 타입을 true로 하면 일반 text로 구분되어 진다.
	   data: form
	}).done(function(data){
