...
var createTable = function() {
  ...
  if (dt_basic_table.length) {
    ...
    var dt_basic = dt_basic_table.DataTable({
	  lengthChange: false,
      ajax: {
        url : '/admin/baby/babyQnaList/table',
        dataType : 'json',
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              '<button type="button" class="btn btn-primary btn-sm btn-sm waves-effect waves-float waves-light" onclick="location.href = \'babyQnaAdd?mbsIdx='+full['mbsIdx']+ '\'">수정</button>'
            );
          }
    ...
    $('div.head-label').html('<h4 class="card-title">게시글 목록 <button type="button" id="btnDel" class="btn btn-outline-danger btn-sm me-1">선택삭제</button>'+
                             '<button type="button" onclick="location.href=\'/admin/baby/babyQnaAdd\'" class="btn btn-outline-info btn-sm me-1">새글등록</button></h4>');
...
function btnDisplay(idx) {
	...
	if(confirm('진열을 수정하시겠습니까?')){
		$.ajax({
			url : '/admin/baby/displayBabyQna?mbsIdx='+idx+'&mbsActive='+mbsActive+'&mbsCheck='+mbsCheck,
			type : "get",
			dataType : "json",
			success : function(data) {
				console.log('data============',data);
