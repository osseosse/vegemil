...
var createTable = function() {
  ...
  if (dt_basic_table.length) {
    ...
    var dt_basic = dt_basic_table.DataTable({
	  lengthChange: false,
      ajax: {
        url : '/admin/manage/publicCenter/getPrintAdList',
        dataType : 'json',
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            if(full['tImg']==null)	return '';
      			else	return  '<img style="width:170px;height:120px; margin-bottom:8px;" src="'+ '/web/upload/PAD/' + full['tImg']+'"/>'
    ...
    $('div.head-label').html('<h4 class="card-title">지면 광고'+
                             '<button type="button" onclick="location.href=\'/admin/manage/publicCenter/printADPost\'" class="btn btn-outline-info btn-sm ms-1">새글등록</button></h4>');
...
function btnDisplay(idx) {
	...
	if(confirm('진열을 수정하시겠습니까?')){
		$.ajax({
			url : '/admin/manage/publicCenter/changePrindADOnairStatus?tIdx='+idx+'&tOnair='+tOnair,
			type : "get",
			dataType : "json",
