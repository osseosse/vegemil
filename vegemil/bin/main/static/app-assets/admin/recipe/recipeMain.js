...
var createTable = function() {
  ...
  if (dt_basic_table.length) {
    ...
    var dt_basic = dt_basic_table.DataTable({
	  lengthChange: false,
      ajax: {
        url : '/admin/manage/recipe/recipeList',
        dataType : 'json',
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            if(full['cCate']==null)	return '';
      			else	return '<input type="text"class="form-control text-center" id="cCate'+full['cIdx']+'" value="'+full['cCate']+'">'
      							+'<p hidden>'+full["cCate"]+'</p>';
          }
        },
        {
          targets: 2,
          orderable: false,
          render: function (data, type, full, meta) {
            if(full['cCode']==null)	return '';
      			else	return '<input type="text" class="form-control" id="cCode'+full['cIdx']+'" value="'+full['cCode']+'">'
								+'<p hidden>'+full["cCode"]+'</p>';
          }
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            if(full['cWith']==null)	return '';
      			else	return '<input type="text" class="form-control" id="cWith'+full['cIdx']+'" value="'+full['cWith']+'">'
      					+'<p hidden>'+full["cWith"]+'</p>';
          }
        },
        {
          targets: 4,
          orderable: false,
          render: function (data, type, full, meta) {
            if(full['cSubject']==null)	return '';
      			else	return '<input type="text" class="form-control" id="cSubject'+full['cIdx']+'" value="'+full['cSubject']+'">'
      							+'<p hidden>'+full["cSubject"]+'</p>';
          }
        },
        {
          targets: 5,
          orderable: false,
          render: function (data, type, full, meta) {
            if(full['cContent']==null)	return '';
      			else	return '<textarea rows="5" class="form-control" id="cContent'+full['cIdx']+'"/>'+full["cContent"]+'</textarea>';
          }
        },
        
        {
          ...
          render: function (data, type, full, meta) {
            //return '영상';
            if(full['cYoutubeId']==null)  return '<input type="text" class="form-control" id="cYoutubeId'+full['cIdx']+'" placeholder="유튜브 동영상 url"></input>';
            return (
					'<iframe '+ 
						'width="200" height="150" src="'+full['cYoutubeId']+'"' + 'title="YouTube video player" frameborder="0" '+
						'allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>'+
					'</iframe>'	+
					'<input type="text" class="form-control" id="cYoutubeId'+full['cIdx']+'" value="'+full['cYoutubeId']+'"></input>'
					+'<p hidden>'+full["cYoutubeId"]+'</p>'
					)
          }
        },
          ...
          render: function (data, type, full, meta) {
            ...
            return (
              '<div class="form-check form-switch center-ck">'+
                '<input type="checkbox" class="form-check-input" '+checked+' id="cVactive'+full['cIdx']+ '" name="listOn" onclick="javascript:btnDisplay('+full['cIdx']+')" >'+
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            if(full['cHit']==null)	return '';
      			else	return full['cHit'];
          }
        },
        {
          targets: 10,
    ...
    $('div.head-label').html('<h4 class="card-title">쿡앤쿡 베지밀 레시피 목록 </h4> ');
...
function btnDisplay(idx) {
	...
	let cVactive;
	if($('#cVactive'+idx).is(":checked")){
		cVactive = 1;
	}else{
		cVactive = 0;
	}
	console.log('============cVactive',cVactive)
	
	if(confirm('진열을 수정하시겠습니까?')){
		$.ajax({
			url : '/admin/manage/recipe/changeVactiveStatus?cIdx='+idx+'&cVactive='+cVactive,
			type : "get",
			dataType : "json",
...
function btnSave(idx, action) {
	...
	console.log('idx', idx);
	console.log('cSubject', $('#cSubject'+idx).val());
	console.log('active!!!!!', $('#active').val());
	const form = $('#form');
	let msg;
	...
	if(action == "I") {
		msg = "등록하시겠습니까?";
		$('#cCate').val($('#cate').val());
		$('#cSubject').val($('#subject').val());
		$('#cYoutubeId').val($('#youtubeId').val());
		//$('#cFilename').val($('#src').fileName());
		$('#cContent').val($('#content').val());
		$('#cWith').val($('#with').val());
		$('#cCode').val($('#code').val());
		//$('#cVactive').val($('#active').val());
		$('#action').val(action);
		...
		$('#cCate').val($('#cCate'+idx).val());
		$('#cCode').val($('#cCode'+idx).val());
		$('#cWith').val($('#cWith'+idx).val());
		$('#cSubject').val($('#cSubject'+idx).val());
		$('#cContent').val($('#cContent'+idx).val());
		$('#cYoutubeId').val($('#cYoutubeId'+idx).val());
		$('#cVactive').val($('#cVactive'+idx).val()=="on"?"1":"0");
	...
	if(confirm(msg)) {
		$.ajax({
	       url: '/admin/manage/recipe/postRecipe',
