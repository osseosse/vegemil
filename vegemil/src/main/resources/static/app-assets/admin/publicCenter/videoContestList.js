...
var createTable = function() {
  ...
  if (dt_basic_table.length) {
    ...
    var dt_basic = dt_basic_table.DataTable({
	  lengthChange: false,
      ajax: {
        url : '/admin/manage/publicCenter/getvideoContestList',
        dataType : 'json',
      ...
      columnDefs: [
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            if(full['tYoutubeImg']==null)	return '';
      			else	return  '<img style="width:170px;height:120px; margin-bottom:8px;" src="'+'/web/upload/VC/'+full["tYoutubeImg"]+'"/>'
      							+'<input hidden type="text" class="form-control" id="tYoutubeImg'+full['tIdx']+'" value="'+full['tYoutubeImg']+'">'
      							+'<p> '+full["tYoutubeImg"]+'</p>';
          }
        },
        {
          targets: 4,
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              '<button type="button" class="btn btn-primary btn-sm btn-sm waves-effect waves-float waves-light" onclick="location.href = \'videoContestUpdate?tIdx='+full['tIdx']+ '\'">수정</button>'
            );
          }
    ...
    $('div.head-label').html('<h4 class="card-title">영상공모전'+
                             '<button type="button" onclick="location.href=\'/admin/manage/publicCenter/postVideoContest\'" class="btn btn-outline-info btn-sm ms-1">새글등록</button></h4>');
...
function btnDisplay(idx) {
	...
	if(confirm('진열을 수정하시겠습니까?')){
		$.ajax({
			url : '/admin/manage/publicCenter/changeContestOnairStatus?tIdx='+idx+'&tOnair='+tOnair,
			type : "get",
			dataType : "json",
...
function btnSave(idx, action) {
	...
	if(confirm(msg)) {
		$.ajax({
	       url: '/admin/manage/publicCenter/saveVideoContest',
