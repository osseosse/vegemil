...
import com.vegemil.domain.*;
import com.vegemil.mapper.MemberNoticeMapper;
...
public class MailService {
    ...
    public void marketingInfoReceiveAgreeConfirm(MemberDTO member) {
        ...
        try {
            ...
            mailSender.send(message);
            upsertMemberNoticeData(new MemberNoticeDTO(member.getMId(), MemberNoticeDTO.NoticeType.IMMEDIATE_MARKETING_CONSENT_CHANGE));
    ...
    // 묹자 메일 마케팅 정보 수신 동의 메일
    public void marketingInfoNoticePer2year(MemberDTO member) {
        ...
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 a HH:mm:ss");
            context.setVariable("agreeDate", sdf.format(new Date()));
            context.setVariable("mId", member.getMId());
            context.setVariable("mSmssend", member.getMSmssend());
            context.setVariable("mEmailsend", member.getMEmailsend());

            message.setFrom(MailService.FROM_ADDRESS);
            message.addRecipients(MimeMessage.RecipientType.TO, member.getMEmail());
            message.setSubject("[정식품] 광고성 정보 수신동의 내역 안내");

            message.setText(templateEngine.process("admin/email/marketing-agreement-notice", context), "utf-8", "html");
            mailSender.send(message);
            upsertMemberNoticeData(new MemberNoticeDTO(member.getMId(), MemberNoticeDTO.NoticeType.BIENNIAL_MARKETING_CONSENT_NOTICE));
    ...
    public void alertSubmitCpDecl(ClaimDTO claimDTO, List<String> recipientsId) {
        ...
        try {
            ...
            context.setVariable("cYear", claimDTO.getCWritedate()
                                                 .split("-")[0]);
            context.setVariable("cMonth", claimDTO.getCWritedate()
                                                  .split("-")[1]);
            context.setVariable("cDay", claimDTO.getCWritedate()
                                                .split("-")[2]);
    ...
    public int upsertMemberNoticeData(MemberNoticeDTO memberNoticeDTO) {

        MemberNoticeDTO byMid = memberNoticeMapper.findByMidAndNoticeType(memberNoticeDTO);
        if (byMid != null && byMid.getIdx() != null) {
            //update
            return memberNoticeMapper.update(memberNoticeDTO);
        } else {
            //save
            return memberNoticeMapper.save(memberNoticeDTO);
        }
    }
